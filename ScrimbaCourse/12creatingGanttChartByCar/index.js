...
function drawChart() {
    ...
    var y = d3.scaleBand()
        .domain(data.map(function(x) { return x.carro }))
        .range([0, height])
        .paddingInner(0.3)
        .paddingOuter(1)
...
function rectTransform(data, x, y) {
    return "translate(" + x(data.saida) + "," + y(data.carro) + ")";
};
...
function dataFilter(data) {
    data = data.filter(element => element.itinerario !== 'Viagem de deslocamento' || element.itinerario === 'Viagem de Deslocamento entre Terminais')
